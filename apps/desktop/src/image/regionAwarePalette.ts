...
function collapseGameEquivalentColoursLite(
  pixelMap: PixelMap,
  ...
  let working = pixelMap;
  let distinct = getDistinctColorHexes(working);

  if (distinct.length <= 1) {
    return reindexPixelMapByColorHex(working);
  }

  // v4-lite 不按用户选的 128 直接保留 128 色，而是按实际复杂度给一个软上限。
  // 但它只合并“游戏里大概率等效”的近似色，不强行砍到固定数量。
  const softCap = Math.min(
    options.targetColorCount,
    Math.max(
      options.targetColorCount <= 64 ? 42 : 52,
      Math.round(Math.sqrt(distinct.length) * 4.2 + 18),
    ),
  );
  ...
  while (guard < 220) {
    distinct = getDistinctColorHexes(working);
    if (distinct.length <= softCap) {
      break;
    }

    const stats = buildLiteColourStats(working);
    let bestPair: [string, string] | null = null;
    let bestScore = Number.POSITIVE_INFINITY;

    for (let i = 0; i < distinct.length; i += 1) {
      const leftHex = distinct[i];
      if (!leftHex) continue;

      for (let j = i + 1; j < distinct.length; j += 1) {
        const rightHex = distinct[j];
        if (!rightHex) continue;

        const left = stats.get(leftHex);
        const right = stats.get(rightHex);
        if (!left || !right) continue;

        if (!canMergeGameEquivalentLite(left, right, options)) {
          continue;
        }

        const score = gameEquivalentLiteDistance(left, right);

        if (score < bestScore) {
          bestScore = score;
          bestPair = [leftHex, rightHex];
        }
      }
    }
    ...
    const left = stats.get(bestPair[0]);
    const right = stats.get(bestPair[1]);

    if (!left || !right) {
      break;
    }

    const [fromHex, toHex] = chooseLiteMergeDirection(left, right);
    working = replaceColourLite(working, fromHex, toHex);
    guard += 1;
  }
...
function buildLiteColourStats(pixelMap: PixelMap): Map<string, LiteColourStats> {
  const stats = new Map<string, LiteColourStats>();
...
function canMergeGameEquivalentLite(
  left: LiteColourStats,
  right: LiteColourStats,
  options: NormalizedOptions,
): boolean {
  const lumaGap = Math.abs(left.luma255 - right.luma255);
  const chromaGap = Math.abs(left.chroma - right.chroma);
  const hueGap = hueDistance(left.hsv.h, right.hsv.h);
  const bothLowChroma = left.chroma < 18 && right.chroma < 18;
  const bothDark = left.luma255 <= 70 && right.luma255 <= 70;
  const oneVeryDark = left.luma255 <= 38 || right.luma255 <= 38;

  // 避免把黑色线稿和明显更亮的填充/高光合并。
  if (oneVeryDark && lumaGap > 10) {
    return false;
  }

  // 有色装饰不要跨色相乱合并，比如金色、粉色、灰蓝色。
  if (!bothLowChroma && hueGap > 18) {
    return false;
  }

  // 64 色比 128 色更谨慎一点，避免出现硬色块。
  const maxLumaGap =
    options.targetColorCount >= 128
      ? bothDark ? 12 : 10
      : bothDark ? 10 : 8;

  const maxChromaGap =
    options.targetColorCount >= 128 ? 18 : 14;

  if (lumaGap > maxLumaGap) {
    return false;
  }

  if (chromaGap > maxChromaGap) {
    return false;
  }

  return gameEquivalentLiteDistance(left, right) <=
    (options.targetColorCount >= 128 ? 18 : 15);
}

function gameEquivalentLiteDistance(left: LiteColourStats, right: LiteColourStats): number {
  const rgbGap =
  ...
  const chromaGap = Math.abs(left.chroma - right.chroma);
  const hueGap = hueDistance(left.hsv.h, right.hsv.h);
  const huePenalty = left.chroma < 18 && right.chroma < 18 ? 0 : hueGap * 0.12;

  return rgbGap * 0.18 + lumaGap * 1.15 + chromaGap * 0.35 + huePenalty;
}

function chooseLiteMergeDirection(
  left: LiteColourStats,
  right: LiteColourStats,
): [string, string] {
  // 深色线条类：尽量保留更暗的那个，避免线稿变浅。
  if (left.luma255 <= 45 || right.luma255 <= 45) {
    return left.luma255 <= right.luma255
      ? [right.hex, left.hex]
      : [left.hex, right.hex];
  }

  // 其他颜色：小面积合并到大面积，减少换色和碎色。
  return left.count <= right.count
    ? [left.hex, right.hex]
    : [right.hex, left.hex];
}

function replaceColourLite(pixelMap: PixelMap, fromHex: string, toHex: string): PixelMap {
  const source = normalizeHex(fromHex);
